 ...
 */
export class FirmwareUpdateRequestMessage extends ExtendedMessage {
  ///< Raw payload bytes after headers.
  public readonly rawPayload: Uint8Array
  ///< Firmware update request data block bytes.
  public readonly firmwareUpdateRequestDataBlock: Uint8Array
  ...
  /**
   * Create a Firmware_Update_Request message.
   *
  ...
  public constructor(
    ...
    if (payload.length < dataEnd) {
      this.parseErrors.push(
        `Firmware_Update_Request expected ${this.dataSize} bytes but only ${payload.length - this.payloadOffset} available`,
      )
    }
    this.firmwareUpdateRequestDataBlock = payload.subarray(
      this.payloadOffset,
  ...
  public override get humanReadableMetadata() {
    ...
    metadata.baseInformation.insertEntryAt(1, 'Message Description', HumanReadableField.string('Firmware_Update_Request is an extended message used to initiate or advance firmware update transactions so partners can perform standardized in-band firmware update procedures.', 'A description of the message\'s function and usage.'))
    return metadata
  }
