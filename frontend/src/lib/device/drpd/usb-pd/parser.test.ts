...
describe('usb-pd parser', () => {
  it('parses SOP\' GoodCRC control messages', () => {
    ...
    expect(startTimestamp?.Label).toBe('Device Timestamp')
    expect(startTimestamp?.value).toBe('0')
    const wallClockTimestamp = timingInformation?.getEntry('wallClockTimestamp')
    expect(wallClockTimestamp?.Label).toBe('Wall Clock Timestamp')
