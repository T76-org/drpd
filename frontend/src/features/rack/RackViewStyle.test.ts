...
describe('RackView responsive header CSS', () => {
  ...
  it('keeps the front-panel visual compact while states change through data hooks', () => {
    ...
    expect(rackViewCss).toMatch(/\.headerFrontPanel\[data-usb-ports-enabled='true'\]\s+\.headerUsbCPort\[data-connected='false'\]\s*\{[^}]*animation:\s*headerUsbCPortAvailablePulse 2\.8s ease-in-out infinite;/s)
    expect(rackViewCss).toMatch(/@media\s*\(prefers-reduced-motion:\s*reduce\)\s*\{[^}]*animation:\s*none;/s)
    expect(rackViewCss).toMatch(/\.headerFrontPanel\[data-disabled='true'\]\s+\.headerBananaJack\s*\{/s)
