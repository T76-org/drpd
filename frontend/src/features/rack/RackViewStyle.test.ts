...
describe('RackView responsive header CSS', () => {
  ...
  it('lets header contents distribute across the available status header width', () => {
    ...
    expect(rackViewCss).toMatch(/\.headerVbusMetrics\s*\{[^}]*justify-content:\s*flex-start;/s)
    expect(rackViewCss).toMatch(/\.headerVbusMetrics\s*\{[^}]*flex:\s*1 1 auto;/s)
    expect(rackViewCss).toMatch(/\.headerVbusStatusGrid\s*\{[^}]*flex:\s*0 0 auto;/s)
  })
  ...
  it('keeps the front-panel visual compact while states change through data hooks', () => {
    ...
    expect(rackViewCss).toMatch(/\.headerFrontPanel\s*\{[^}]*position:\s*relative;/s)
    expect(rackViewCss).toMatch(/\.headerFrontPanelDisabledBadge\s*\{[^}]*display:\s*none;/s)
    expect(rackViewCss).toMatch(/\.headerFrontPanelDisabledBadge\s*\{[^}]*position:\s*absolute;/s)
    expect(rackViewCss).not.toMatch(/\.headerFrontPanelDisabledBadge\s*\{[^}]*border:/s)
    expect(rackViewCss).toMatch(/\.headerFrontPanel\[data-role='DISABLED'\]\s+\.headerFrontPanelDisabledBadge\s*\{[^}]*display:\s*block;/s)
