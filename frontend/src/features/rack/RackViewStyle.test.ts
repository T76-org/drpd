...
describe('RackView responsive header CSS', () => {
  ...
  it('keeps the front-panel visual compact while states change through data hooks', () => {
    ...
    expect(rackViewCss).toMatch(/\.headerFrontPanel\[data-port-rail-direction='port-2-to-port-1'\]\s+\.headerFrontPanelPortRailLine\s*\{[^}]*animation-direction:\s*reverse;/s)
