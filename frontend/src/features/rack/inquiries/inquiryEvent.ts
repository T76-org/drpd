...
export const presentInquiryResponse = (request: SinkInquiryRequest, result: Extract<InquiryRunState, { phase: 'response' }>): InquiryEventResult => {
  ...
  if (request.type === SinkInquiryType.GET_MANUFACTURER_INFO) {
    ...
    const interoperabilityWarning = 'The Source omitted the required trailing null terminator. Dr. PD recovered the printable ASCII string using the declared Manufacturer_Info Data Size; raw bytes are preserved unchanged.'
    return { title, summary: [`- **${plug ? `${cablePlugLabel(plug)} cable manufacturer identity` : 'Port manufacturer identity'}:**`, `  - **VID:** ${hex16(info.vid)}`, `  - **PID:** ${hex16(info.pid)}`, `  - **Manufacturer:** ${info.manufacturerString || '(empty)'}`, ...(!nullTerminated ? [`  - **Interoperability warning:** ${interoperabilityWarning}`] : []), ...(plug ? [`  - **Target:** ${cablePlugLabel(plug)}`] : [])].join('\n'), eventData: [{ title: `${plug ? `${cablePlugLabel(plug)} Cable ` : 'Port '}Manufacturer Identity`, entries: [...targetEntry, { key: 'Vendor ID (bytes 0–1)', value: `**${hex16(info.vid)}**; raw little-endian bytes ${rawHexValue(raw.subarray(0, 2))}.` }, { key: 'Product ID (bytes 2–3)', value: `**${hex16(info.pid)}**; raw little-endian bytes ${rawHexValue(raw.subarray(2, 4))}.` }, { key: 'Manufacturer String (bytes 4–end)', value: `**${info.manufacturerString || '(empty)'}**; ${nullTerminated ? 'null-terminated' : 'unterminated'} printable ASCII bytes ${rawHexValue(info.manufacturerStringBytes)}.` }, ...(!nullTerminated ? [{ key: 'Interoperability Warning', value: interoperabilityWarning }] : []), rawEntry] }] }
  }
