...
class DeviceAnalogMonitor:
    ...
    async def get_status(self) -> AnalogMonitorChannels:
        ...
        """
        result = await self._internal.query_ascii_values_and_check("MEAS:ALL?", "s")

        assert len(
            result) == 9, "Expected 9 parameters in the voltage response."

        return AnalogMonitorChannels(
            vbus=float(result[0]),
            ibus=float(result[1]),
            dut_cc1=float(result[2]),
            dut_cc2=float(result[3]),
            usds_cc1=float(result[4]),
            usds_cc2=float(result[5]),
            adc_vref=float(result[6]),
            ground_ref=float(result[7]),
            current_vref=float(result[8]),
        )
